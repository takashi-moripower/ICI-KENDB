...
class ResponsiblesController extends AppController
{
	...
	{
		parent::beforeFilter();
		$this->set("title_for_layout", "担当テーブル");
		$this->crumbList = array(
			...
			array(
				__("Responsibles", true),
				array('controller' => 'responsibles', 'action' => 'index'),
				array()
			)
		);
	}
	...
	{
		$this->_setPullDown();
		$this->Responsible->recursive = 1;
		$this->set('responsible', $this->Responsible->findById($id));

		return $this->_edit("Responsible", $id);
	}
	...
	function upload()
	{
		if (!empty($this->data)) {
			$message = "";
			$error_message = array();
			$this->_registerUploadFile("Responsible", "id", $message, $error_message);
	...
	function output_excel()
	{
		...
		$this->_makeExcelObject("Responsible", $rec, $format);
		return;
	}
	...
	function output_csv()
	{
		...
		$rec = $this->paginate();
		$format = $this->_getExcelFormat();
		$this->_makeCSVObject("Responsible", $rec);
		return;
	}
	...
	function _registerUploadFile($modelname, $primaryKey, &$message, &$error_message)
	{
		...
		$this->$modelname->query("DELETE FROM responsibles");
		foreach ($body as $item) {
			$line++;
			$tmp = $this->$modelname->columns2data($item, $modelname);
			$researcher_rec = $Researcher->findByPersonalNo(sprintf('%08d', $tmp[$modelname]['personal_no']));
