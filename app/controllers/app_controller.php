...
class AppController extends Controller {
     ...
     * @return None
     */
    function _makeExcelObject0($modelname, $datalist, $exceltype = "Excel5") {
        Configure::write("debug", 0);
        App::import('vendor', 'phpexcel/phpexcel');
        $excel = new PHPExcel();
        // シートの設定
    ...
    function _registerUploadFile($modelname, $primaryKey, &$message, &$error_message) {
        ...
        if (!$header_is_same) {
            ...
            $message = __('There is no header record or does not match column count.', true);
            return false;
        }
        ...
        if ($rollback) {
            $message = sprintf(__('Total %d records. %d records are invalid! rollback', true), $line, $line - $success_count);
            return false;
        } else {
            if ($line - $success_count == 0) {
                $message = sprintf(__('Total %d records. %d records have been imported!', true), $line, $success_count);
            } else {
                $message = sprintf(__('Total %d records. %d records have been imported. but %d records failed to insert or update.', true), $line, $success_count, $line - $success_count);
            }
        }
        ...
        if (!empty($this->data)) {
            ...
            if ($this->$modelname->save($this->data)) {
                $this->Session->setFlash(
                        sprintf(
                                __('The %s has been saved', true), __(Inflector::humanize(Inflector::tableize($modelname)), true)));
                $this->redirect(array('action' => 'index'));
                return;
            } else {
                $this->Session->setFlash(
                        sprintf(
                                __('The %s could not be saved. Please, try again.', true), __(Inflector::humanize(Inflector::tableize($modelname)), true)));
            }
        } else {
    ...
    function _add($modelname) {
        ...
        if (!empty($this->data)) {
            ...
            if ($this->$modelname->save($this->data)) {
                $this->Session->setFlash(
                        sprintf(__('The %s has been saved', true), __(Inflector::humanize(Inflector::tableize($modelname)), true)));
                $this->redirect(array('action' => 'index'));
            } else {
                $this->set('validate_errors', $this->validateErrors($this->$modelname));
                $this->Session->setFlash(
                        sprintf(__('The %s could not be saved. Please, try again.', true), __(Inflector::humanize(Inflector::tableize($modelname)), true)));
            }
        } else {
    ...
    function _edit($modelname, $id) {
        ...
        if (!$id && empty($this->data)) {
            ...
            $this->Session->setFlash(
                    sprintf(__('Invalid %s', true), __(Inflector::humanize(Inflector::tableize($modelname)), true)));
            $this->redirect(array('action' => 'index'));
        }
        if (!empty($this->data)) {
    ...
    function _delete($modelname, $id) {
        ...
        if (!$id) {
            $this->Session->setFlash(
                    sprintf(__('Invalid id for %s', true), __(Inflector::humanize(Inflector::tableize($modelname)), true)));
            $this->redirect(array('action' => 'index'));
        }
        ...
        if ($rec && $rec[$modelname]["disabled"] == 1) {
            $this->Session->setFlash(
                    sprintf(__('%s deleted', true), __(Inflector::humanize(Inflector::tableize($modelname)), true)));
            $this->redirect(array('action' => 'index'));
        } else {
            $this->Session->setFlash(
                    sprintf(__('%s was not deleted', true), __(Inflector::humanize(Inflector::tableize($modelname)), true)));
            $this->redirect(array('action' => 'index'));
        }
    }
