<div id="snavi">
	<ul>
		...
		<li><a href="<?php echo $html->url('/mhlw_research_grants/output_excel') . $urlparams . "/format:Excel5" ?>">Excel形式ダウンロード(.xls)</a></li>
		<li><a href="<?php echo $html->url('/mhlw_research_grants/output_excel') . $urlparams . "/format:Excel2007" ?>">Excel2007形式ダウンロード(.xlsx)</a></li>
		<?php endif; ?>
		<?php if($can_output_csv): ?>
		<li><a href="<?php echo $html->url('/mhlw_research_grants/output_csv') . $urlparams; ?>">CSV形式ダウンロード</a></li>
		<?php endif; ?>
	</ul>
</div>
...
function search_box_change(n) {
	...
	var url = "<?php echo $html->url('/mhlw_research_grants/search_box/'); ?>";
...
<?php echo $this->element("table_switcher_head", array("count" => count($mhlwResearchGrants))); ?>

<div class="mhlwResearchGrants index">
	<h2><?php __('Mhlw Research Grants');?></h2>
	<?php echo $this->element("data_search", array("modelname" => "MhlwResearchGrant")); ?>
	...
	<tr>
		...
		<th width="65"><?php echo $this->Paginator->sort('this_year_advance_application_date'); ?></th>
		<th width="70"><?php echo $this->Paginator->sort('moving_history'); ?></th>
		<th width="30"><?php echo $this->Paginator->sort('commission_coi_check'); ?></th>
		<th width="100"><?php echo $this->Paginator->sort('business_name'); ?></th>
		<th width="50"><?php echo $this->Paginator->sort('titech_assignment_no'); ?></th>
		<th width="50"><?php echo $this->Paginator->sort('assignment_no'); ?></th>
		<th width="50"><?php echo $this->Paginator->sort('new_or_continuance'); ?></th>
	...
	<?php
	$i = 0;
	foreach ($mhlwResearchGrants as $mhlwResearchGrant):
		$class = null;
		if($mhlwResearchGrant["MhlwResearchGrant"]["disabled"]) {
			$class = ' class="disabled"';
		}
		$item = $mhlwResearchGrant["MhlwResearchGrant"];
		$editing = "";
	...
	<tr<?php echo $class;?>>
		<td class="actions">
			<?php echo $this->Html->link(__('View', true), array('action' => 'view', $mhlwResearchGrant['MhlwResearchGrant']['id'], '?rtn=' . $_SERVER["REQUEST_URI"])); ?>
			<?php if($can_edit): ?>
			<?php echo $this->Html->link(__('Edit', true), array('action' => 'edit', $mhlwResearchGrant['MhlwResearchGrant']['id'], '?rtn=' . $_SERVER["REQUEST_URI"])); ?>
			<?php endif; ?>
			<?php echo $editing; ?>
		</td>
		...
		<td><?php echo h($item['commission_coi_check']); ?>&nbsp;</td>
		...
		<?php echo $this->Kendb->writeCell($mhlwResearchGrant, "MhlwResearchGrant", 'this_year_application_amount', null, "num", "number"); ?>
		<?php echo $this->Kendb->writeCell($mhlwResearchGrant, "MhlwResearchGrant", 'this_year_total_at_decision', null, "num", "number"); ?>
		<?php echo $this->Kendb->writeCell($mhlwResearchGrant, "MhlwResearchGrant", 'this_year_direct_at_decision', null, "num", "number"); ?>
		<?php echo $this->Kendb->writeCell($mhlwResearchGrant, "MhlwResearchGrant", 'this_year_indirect_at_decision', null, "num", "number"); ?>
		<?php echo $this->Kendb->writeCell($mhlwResearchGrant, "MhlwResearchGrant", 'this_year_application_amount_sum', null, "num", "number"); ?>
		<?php echo $this->Kendb->writeCell($mhlwResearchGrant, "MhlwResearchGrant", 'this_year_direct_cost', null, "num", "number"); ?>
		<?php echo $this->Kendb->writeCell($mhlwResearchGrant, "MhlwResearchGrant", 'this_year_indirect_cost', null, "num", "number"); ?>
		<td><?php echo $this->Kendb->shorten_string($item['research_assignment']); ?>&nbsp;</td>
		<td><?php echo h($item['contribution_count']); ?>&nbsp;</td>
		<?php echo $this->Kendb->writeCell($mhlwResearchGrant, "MhlwResearchGrant", 'contribution_partition', null, "num", "number"); ?>
