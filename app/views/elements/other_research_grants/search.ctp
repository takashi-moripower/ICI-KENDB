	<?php echo $this->KendbForm->create('OtherResearchGrant', array('url' => array_merge(array('action' => 'index'), $this->params['pass']))); ?>
	...
	<td colspan="2" class="colspan" nowrap><?php echo $this->KendbForm->input('paying_for_day_f', array('div' => false, 'label' => __('Paying For Day' , true))); ?>〜<?php echo $this->KendbForm->input('paying_for_day_t', array('div' => false, 'label' => false)); ?></td>
	<tr>
	</table>
	<table border="0" cellspacing="4" class="entrust_search_table">
	<tr valign="top">
...
function set_rinfo(id) {
	$.get("/researchers/getJsonById/"+id, function(data) {
		...
		$("#OtherResearchGrantName").val(researcher.researcher_name);
