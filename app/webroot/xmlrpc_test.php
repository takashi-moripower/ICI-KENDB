...
function test_データベースと検索するとそれ以外も返ってくるバグ() {
    ...
    if (!$client->query('kendb.search', array(), '5', $search_cond, array())) { 
...
function test_ANDの位置がどこでも良いこと() {
    ...
    if (!$client->query('kendb.search', array(), '5', $search_cond2, array())) { 
...
function test_不正な結果がかえってくる問題() {
    ...
    if (!$client->query('kendb.search', array(), '1', $search_cond, array(), $order, 0, 20)) {
        echo('Something went wrong - '.$client->getErrorCode().' : '.$client->getErrorMessage());
        echo "\n";
    } else {
        echo "\n---\n";
...
function test_東工大連携IDで横断検索() {
    global $client;
    if (!$client->query('kendb.get', array(), '0', array("000001618-7", "000001186-X", "000000000-7", "000000001-S"), array())) { 
        echo('Something went wrong - '.$client->getErrorCode().' : '.$client->getErrorMessage()); 
        echo "\n";
}    else {
        echo "\n---\n"; 
...
function test_ID指定し応答フィールド指定なし() {
    global $client;
    if (!$client->query('kendb.get', array(), '1', array(2151,2152,2153,2154), array())) { 
        echo('Something went wrong - '.$client->getErrorCode().' : '.$client->getErrorMessage()); 
        echo "\n";
    } else {
        echo "\n---\n"; 
...
function test_ID指定しデータ取得応答フィールド指定あり() {
    global $client;
    if (!$client->query('kendb.get', array(), '1', array(2151,2152,2153,2154), array('id', 'year', 'amount'))) { 
        echo('Something went wrong - '.$client->getErrorCode().' : '.$client->getErrorMessage()); 
        echo "\n";
    } else {
        echo "\n---\n"; 
...
function test_単純検索応答フィールド指定あり() {
    ...
    if (!$client->query('kendb.search', array(), '5', $search_cond, array('id', 'year', 'amount'))) { 
        echo('Something went wrong - '.$client->getErrorCode().' : '.$client->getErrorMessage()); 
        echo "\n";
    } else {
        echo "\n---\n"; 
...
function test_教員検索で応答フィールド指定なし多重階層() {
    ...
    if (!$client->query('kendb.search', array(), '1', $search_cond, array())) { 
        echo('Something went wrong - '.$client->getErrorCode().' : '.$client->getErrorMessage()); 
        echo "\n";
    } else {
        echo "\n---\n"; 
...
function test_検索で応答フィールド指定なし条件多階層ソート順指定件数指定なし() {
    ...
    if (!$client->query('kendb.search', array(), '1', $search_cond, array(), $order)) { 
        echo('Something went wrong - '.$client->getErrorCode().' : '.$client->getErrorMessage()); 
        echo "\n";
    } else {
        echo "\n---\n"; 
...
function test_データ件数取得API() {
    ...
    if (!$client->query('kendb.count', array(), '1', $search_cond)) { 
        echo('Something went wrong - '.$client->getErrorCode().' : '.$client->getErrorMessage()); 
        echo "\n";
    } else {
        echo "\n---\n"; 
